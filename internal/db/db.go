...
func (db *DB) GetGroupData(groupId string) (groupData GroupData, err error) {
	err = db.conn.QueryRow(context.Background(), "SELECT id, tg_id, name, isgroup, date_last_use, sub_end_date, time_last_update FROM group_data WHERE tg_id=$1", groupId).Scan(
...
func (db *DB) GetGroupsOfUser(userId string, groupList *list.List, onlyGroup bool) error {
	rows, err := db.conn.Query(context.Background(), "SELECT g.id, g.tg_id, g.name, g.isgroup, g.date_last_use, g.sub_end_date, g.time_last_update FROM group_data g JOIN users_of_group ug ON ug.group_id=g.id JOIN user_data u ON u.id=ug.user_id WHERE u.tg_id=$1", userId)
	if err != nil {
		return err
	}
	defer rows.Close()
